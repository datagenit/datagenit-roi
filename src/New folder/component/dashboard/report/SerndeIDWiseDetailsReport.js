...
const SerndeIDWiseDetailsReport = () => {
  let { sender, dateform, dateto } = useParams();
  ...
  const detailsReport = () => {
    ...
    fetch(
      `${URL}report.php?user_id=${userid}&method=details&date_from=${DateFrom}&date_to=${DateTo}&senderid=${senderid}&mobile=${mobile}&token=${token}&user_type=${type}`
    ).then((response) => {
  ...
  const deliveredReport = (message_id) => {
    setdeliveredReportData([]);
    fetch(
      `${URL}report.php?user_id=${userid}&method=delivery_status&message_id=${message_id}&token=${token}&user_type=${type}`
    ).then((response) => {
      response.json().then((result) => {
        if (result.success === true) {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (e) => (
        <span
          onClick={() => deliveredReport(e.message_id)}
          data-toggle="modal"
          role="button"
...
export default SerndeIDWiseDetailsReport;
