...
const SummaryReport = () => {
    ...
    const summaryReport = () => {
        ...
        fetch(`${URL}report.php?user_id=${userid}&method=summarynew&date_from=${DateFrom}&date_to=${DateTo}&senderid=${selectedSernderId}&token=${token}&user_type=${type}`).then((response) => {
    ...
    const columns = [
        {
            name: 'User Name',
            cell: (item) => <Link to={'/dashboard/serderid-details-report/'+ item.sernderid + '/' + DateFrom + '/' + DateTo} className='text-primary'>{item.username} </Link>,
        ...
        {
            name: 'Sender ID',
            cell: (item) => <Link to={'/dashboard/serderid-details-report/'+ item.sernderid + '/' + DateFrom + '/' + DateTo} className='text-primary'>{item.sernderid} </Link>,
        ...
        {
            name: 'Total Sent',
            cell: (item) => <span className='text-success'>{parseInt(item.count.delivered || 0) + parseInt(item.count.failed || 0) + parseInt(item.count.other || 0)} </span>,
            selector: row => parseInt(row.count.delivered || 0) + parseInt(row.count.failed || 0) + parseInt(row.count.other || 0),
            sortable: true,
        },
        {
            name: 'Total Delivered',
            cell: (e) => <span className='text-success'>{e.count.delivered || 0} </span>,
                                                ...
                                                <div className="col-md-3">
                                                    <div className="input-group">
                                                        ...
                                                        <select disabled  className='form-select'>
                                                            <option value="sernderWise"> Sender ID Wise </option>
                                                ...
                                                <div className="col-md-3">
                                                    ...
                                                    <div className="text-right">
                                                        {isloading ?
                                                            ...
                                                            :
                                                            <button onClick={summaryReport} className="btn btn-block btn-primary me-1">
                                    ...
                                    <div className="">
                                        <div className="card pb-5">
                                            ...
                                            <DataTable
                                                ...
                                                noDataComponent={'Click on search button to get data Sender ID'}
                                            />
